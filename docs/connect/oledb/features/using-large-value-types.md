---
title: Использование типов больших значений
description: Сведения о поддержке использования типов данных с большими значениями (varchar (max), varbinary (max) и nvarchar (max)) в OLE DB Driver for SQL Server.
ms.custom: ''
ms.date: 06/12/2018
ms.prod: sql
...
Ранее только типы данных [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)], включая **text**, **ntext** и **image**, могли достигать такой длины. Описатель **max** для типов данных **varchar**, **nvarchar** и **varbinary** сделали эти типы данных избыточными. Однако поскольку типы данных большой длины до сих пор доступны, большинство интерфейсов к компонентам доступа к данным OLE DB остаются теми же. Для обратной совместимости с более ранними версиями флаг DBCOLUMNFLAGS_ISLONG сохраняется в OLE DB Driver for SQL Server. Поставщики и драйверы, предназначенные для [!INCLUDE[ssVersion2005](../../../includes/ssversion2005-md.md)] и более поздних версий, продолжают использовать эти термины для новых типов при задании неограниченной максимальной длины.
...
Типы данных **varchar(max)** , **varbinary(max)** и **nvarchar(max)** в столбцах с размером **max**, для которых установлено неограниченное значение, представляются как ISLONG через основные наборы строк схемы OLE DB и интерфейсы, которые возвращают типы столбцов.
...
Чтобы поддерживать потоковую передачу, драйвер OLE DB для SQL Server требует, чтобы доступ к параметрам переменной длины осуществлялся последовательно. Это требование означает, что для свойства DBPROP_ACCESSORDER необходимо задавать значение DBPROPVAL_AO_SEQUENTIALSTORAGEOBJECTS или DBPROPVAL_AO_SEQUENTIAL каждый раз, когда столбцы **varchar(max)** , **nvarchchar(max)** или **varbinary(max)** либо выходные параметры привязываются к DBTYPE_IUNKNOWN. Если не следовать этому ограничению на порядок доступа, то вызов метода **IRowset::GetData** завершится с ошибкой DBSTATUS_E_UNAVAILABLE. Это ограничение не применяется, если нет выходных привязок с использованием типа DBTYPE_IUNKNOWN.
...
- Привязать как тип, имеющий поддерживаемые привязки к базовому типу столбца (например, для nvarchar(max) выполните привязку, указав тип, который можно привязать к типу nvarchar). Если буфер недостаточно большой, то происходит усечение точно так же, как и для базового типа, хотя большие значения теперь доступны.
...
Сообщая максимальный размер столбца, OLE DB Driver for SQL Server возвращает:
...
При получении типов больших значений наиболее эффективным подходом является привязка как типа DBTYPE_IUNKNOWN и задание для свойства DBPROP_ACCESSORDER набора строк значения DBPROPVAL_AO_SEQUENTIALSTORAGEOBJECTS. Этот параметр вызовет передачу значения в потоке напрямую из сети без промежуточной буферизации, как показано в следующем примере:
...
Дополнительные сведения о том, как OLE DB Driver for SQL Server представляет типы больших значений, см. в статье [Большие двоичные объекты и объекты OLE](../../oledb/ole-db-blobs/blobs-and-ole-objects.md).
