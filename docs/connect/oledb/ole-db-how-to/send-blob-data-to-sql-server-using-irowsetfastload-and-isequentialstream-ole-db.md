---
title: Отправка данных BLOB на SQL Server с помощью интерфейсов IROWSETFASTLOAD и ISEQUENTIALSTREAM | Документы Майкрософт
description: Узнайте, как использовать интерфейс IRowsetFastLoad для потоковой передачи данных большого двоичного объекта (BLOB) переменной длины по одной строке в SQL Server. В этом примере также демонстрируется использование ISequentialStream.
ms.custom: ''
ms.date: 05/25/2020
ms.prod: sql
...
author: David-Engel
ms.author: v-daenge
ms.openlocfilehash: ddb9cc49b22618f4d8cb8077b701d50d81197aab
ms.sourcegitcommit: 0310fdb22916df013eef86fee44e660dbf39ad21
ms.translationtype: HT
ms.contentlocale: ru-RU
ms.lasthandoff: 03/20/2021
ms.locfileid: "104741104"
---
...
[!INCLUDE[Driver_OLEDB_Download](../../../includes/driver_oledb_download.md)]
 ...
 По умолчанию этот образец показывает, как использовать интерфейс IRowsetFastLoad для потоковой передачи данных большого двоичного объекта (BLOB) переменной длины по одной строке с использованием встроенных привязок. Встроенные данные большого двоичного объекта должны умещаться в доступной памяти. Этот метод наиболее эффективен, когда размер данных большого двоичного объекта не превышает нескольких мегабайтов, поскольку при этом отсутствуют дополнительные потоковые затраты. Для данных, превышающих несколько мегабайтов, особенно данных, недоступных поблочно, поток обеспечивает более высокую производительность.  
  
 Если в исходном коде раскомментировать `#define USE_ISEQSTREAM`, в образце будет использоваться ISequentialStream. Реализация потока определена в образце и обеспечивает передачу данных BLOB-объекта простым изменением MAX_BLOB. Данные потока не обязательно должны умещаться в памяти или быть доступными одним блоком. Этот поставщик вызывается с помощью метода IRowsetFastLoad::InsertRow. Передайте указатель, используемый методом IRowsetFastLoad::InsertRow в реализацию потока в буфере данных (смещение rgBinding.obValue) вместе с количеством данных, доступных для чтения из потока. Некоторым поставщикам не обязательно знать длину данных при выполнении привязки. В этом случае длина может быть опущена из привязки.  
  
 Образец не использует потоковый интерфейс поставщика для записи данных в поставщик. Вместо этого образец передает указатель на объект потока, который поставщик будет использовать для чтения данных. Как правило, поставщики Майкрософт (SQLOLEDB, SQLNCLI и MSOLEDBSQL) будут читать данные 1024-байтовыми фрагментами. Поставщики считывают данные из объекта до тех пор, пока все данные не будут обработаны. Ни SQLOLEDB, ни SQLNCLI, ни MSOLEDBSQL не располагают полными реализациями, позволяющими потребителю записывать данные в объект потока поставщика. Только данные нулевой длины могут быть переданы через объект потока поставщика.  
  
 Реализованный потребителем объект ISequentialStream может быть использован с данными набора строк (IRowsetChange::InsertRow, IRowsetChange::SetData). Кроме того, его можно использовать с параметрами, привязывая параметр как DBTYPE_IUNKNOWN.  
  
 Поскольку DBTYPE_IUNKNOWN указан как тип данных в привязке, он должен соответствовать типу столбца или целевого параметра. Преобразования невозможны при передаче данных через интерфейс ISequentialStream из интерфейсов набора строк <a href="#conversion_note"><sup>**1**</sup></a>. Для параметров следует избегать использования ICommandWithParameters::SetParameterInfo. Укажите другой тип для принудительного преобразования. Это потребует от поставщика локального кэширования всех данных больших двоичных объектов, чтобы преобразовать их перед отправкой в SQL Server. Кэширование крупного двоичного объекта и его локальное преобразование не обеспечивают хорошей производительности.  

 Дополнительные сведения см. в статье [Большие двоичные объекты и объекты OLE](../../oledb/ole-db-blobs/blobs-and-ole-objects.md).

 <b id="conversion_note">[1].</b> Хотя преобразования невозможны, переводы между UTF-8 и кодовой страницей параметров сортировки базы данных по-прежнему могут возникать, если сервер не поддерживает UTF-8. Дополнительные сведения см. в разделе [Поддержка UTF-8 в OLE DB Driver for SQL Server](../features/utf-8-support-in-oledb-driver-for-sql-server.md).
