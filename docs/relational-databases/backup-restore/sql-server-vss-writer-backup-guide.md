...
### <a name="re-enabling-and-starting-sql-writer"></a>Повторное включение и запуск модуля записи SQL

По умолчанию модуль записи SQL включен и запускается автоматически. Если эта конфигурация была изменена, для возврата к параметрам по умолчанию необходимо сделать следующее.
