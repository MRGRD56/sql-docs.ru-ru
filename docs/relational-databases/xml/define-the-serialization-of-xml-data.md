...
-   Суррогатная пара преобразуется в цифровую ссылку (только при приведении на сервере). Например, суррогатная пара U+D800 U+DF00 преобразуется в числовую ссылку `&#x00010300;`.  
  
-   Чтобы при синтаксическом анализе защитить от нормализации символы табуляции (TAB, U+0009) и перевода строки (LF, U+000A), в значениях атрибутов они преобразуются в свои числовые ссылки: `&#x9;` и `&#xA;` соответственно.  
