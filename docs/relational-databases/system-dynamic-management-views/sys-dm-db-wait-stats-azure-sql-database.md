...
title: sys.dm_db_wait_stats (база данных SQL Azure)
ms.custom: ''
ms.date: 03/12/2021
...
> [!NOTE]
> Эта статистика не сохраняется после событий отработки отказа базы данных SQL, и все данные являются совокупными с момента последнего сброса статистики или запуска ядра СУБД. Используйте `sqlserver_start_time` столбец в [sys.dm_os_sys_info](sys-dm-os-sys-info-transact-sql.md) , чтобы найти время последнего запуска ядра СУБД.   
...
|SE_REPL_COMMIT_ACK|Происходит, когда транзакция ожидает подтверждения фиксации кворума от вторичных реплик.|  
|SE_REPL_COMMIT_TURN|Происходит, когда транзакция ожидает фиксации после получения подтверждений фиксации кворума.|  
|SE_REPL_ROLLBACK_ACK|Происходит, когда транзакция ожидает подтверждения отката кворума от вторичных реплик.|  
