...
|SQL_ACTIVE_CONNECTIONS|Число соединений ограничено [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]. Драйвер возвращает 0 для этого запроса **SQLGetInfo** .|  
|SQL_ACTIVE_ENVIRONMENTS|Число сред не ограниченно драйвером. Драйвер возвращает 0 для этого запроса **SQLGetInfo** .|  
|SQL_ACTIVE_STATEMENTS|Драйвер возвращает 1 для этого запроса **SQLGetInfo** . Число дескрипторов инструкций, доступных приложению, не ограничивается драйвером, но выполнение по умолчанию на дескрипторе инструкции заблокирует выполнение на любом другом дескрипторе.|  
...
|SQL_CONVERT_DOUBLE|Преобразование типа данных ODBC SQL_DOUBLE не поддерживается. [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]Драйвер ODBC для собственного клиента поддерживает тип данных odbc SQL_DOUBLE SQL_FLOAT. См. раздел SQL_CONVERT_FLOAT далее.|  
...
|SQL_CONVERT_INTERVAL_YEAR_MONTH|Отсутствует поддержка для преобразования типов данных интервала.|  
|SQL_CONVERT_INTERVAL_DAY_TIME|Отсутствует поддержка для преобразования типов данных интервала.|  
...
|SQL_DATA_SOURCE_NAME|Имя текущего источника данных. Задает значение, на которое указывает *стрингленгсптр* , равным 0, если в соединении не указано имя источника данных.|  
...
|SQL_DBMS_VER|Номер версии экземпляра [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)], с которым установлено соединение.|  
|SQL_DEFAULT_TXN_ISOLATION|SQL_TXN_READ_COMMITTED|  
|SQL_DESCRIBE_PARAMETER|«Y»|  
|SQL_DRIVER_NAME|"sqlncli11.dll"|  
|SQL_DRIVER_ODBC_VER|Поддерживаемая версия ODBC драйвера.|  
|SQL_DRIVER_VER|Номер версии драйвера.|  
|SQL_DROP_ASSERTION|FALSE|  
|SQL_DROP_CHARACTER_SET|FALSE|  
|SQL_DROP_COLLATION|FALSE|  
|SQL_DROP_DOMAIN|FALSE|  
|SQL_DROP_SCHEMA|DROP SCHEMA не поддерживается|  
...
|SQL_IDENTIFIER_CASE|SQL_IC_MIXED, при соединении с сервером, имеющим порядок сортировки без учета регистра.<br /><br /> SQL_IC_MIXED, при соединении с сервером, имеющим порядок сортировки с учетом регистра.|  
...
|SQL_ODBC_VER|Номер текущей версии диспетчера драйверов ODBC.|  
...
|SQL_QUOTED_IDENTIFIER_CASE|SQL_IC_MIXED при соединении с сервером, имеющим порядок сортировки без учета регистра.<br /><br /> SQL_IC_MIXED при соединении с сервером, имеющим порядок сортировки с учетом регистра.|  
|SQL_ROW_UPDATES|"N"|  
...
|SQL_SERVER_NAME|Имя сервера, соответствующего соединению.|  
|SQL_SPECIAL_CHARACTERS|Зависит от установленной на [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] кодировки.|  
