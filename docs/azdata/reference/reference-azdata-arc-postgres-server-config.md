...
|Команда|Описание|
| --- | --- |
[azdata arc postgres server config init](#azdata-arc-postgres-server-config-init) | Инициализирует файлы определения настраиваемого ресурса (CRD) и спецификации для группы серверов с Гипермасштабированием для PostgreSQL с поддержкой Azure Arc.
...
Инициализирует файлы определения настраиваемого ресурса (CRD) и спецификации для группы серверов с Гипермасштабированием для PostgreSQL с поддержкой Azure Arc.
```bash
...
#### `--path -p`
Путь, куда должны быть записаны определение настраиваемого ресурса и спецификация для группы серверов с Гипермасштабированием для PostgreSQL с поддержкой Azure Arc.
