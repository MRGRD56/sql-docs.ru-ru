...
|--|--|--|--|
| [CU10](#cu10) |  15.0.4123.1 | 20.3.2    | 06.04.2021 |
...
Выпуск накопительного пакета обновления 10 (CU10) для SQL Server 2019.

|Версия пакета | Тег образа |
|-----|-----|
|15.0.4123.1|[2019-CU10-ubuntu-20.04]|

Накопительный пакет обновления 10 (CU10) для кластеров больших данных SQL Server 2019 включает в себя важные возможности:

- Базовые образы обновлены с версии Ubuntu 16.04 до версии Ubuntu 20.04.
   > [!CAUTION]
   > В Ubuntu 20.04 более строгие требования к безопасности, и при использовании BDC для подключения к экземплярам SQL Server версии до SQL Server 2017 могут возникнуть проблемы. Дополнительные сведения см. в разделе [Не удалось подключиться к удаленному экземпляру SQL Server 2016 или более ранней версии](#failed-to-connect-to-remote-instance-of-sql-server-2016-or-older).
- Поддержка высокого уровня доступности для компонентов KMS Hadoop.
- Дополнительные параметры для настройки сетевого взаимодействия и сходства процессов SQL Server на уровне ресурса. См. раздел [Параметры в области ресурсов главного пула](reference-config-bdc-overview.md#master-pool-resource-scope-settings).
- Управление ресурсами для контейнеров, связанных со Spark, с помощью [параметров в области кластера BDC](reference-config-bdc-overview.md#bdc-cluster-scope-settings).
...
### <a name="failed-to-connect-to-remote-instance-of-sql-server-2016-or-older"></a>Не удалось подключиться к удаленному экземпляру SQL Server 2016 или более ранней версии

- **Затронутый выпуск**: CU10
- **Проблема и последствия для клиентов**. При использовании PolyBase в BDC CU10 для подключения к существующему экземпляру SQL Server, который использует для шифрования канала сертификат, созданный с помощью алгоритма SHA1, может возникнуть следующая ошибка:     
...
- **Решение**. Из-за более строгих требований к безопасности в Ubuntu 20.04 по сравнению с предыдущей версией базового образа удаленное подключение с использованием сертификата на базе алгоритма SHA1 запрещается. Самозаверяющий сертификат SQL Server выпусков 2005–2016 по умолчанию использовал алгоритм SHA1. Дополнительные сведения об изменениях, внесенных в самозаверяющие сертификаты в SQL Server 2017, см. в [этой записи блога](https://techcommunity.microsoft.com/t5/sql-server-support/changes-to-hashing-algorithm-for-self-signed-certificate-in-sql/ba-p/319026). В удаленном экземпляре SQL Server используйте сертификат, который создан с помощью алгоритма, имеющего по крайней мере 112-битный уровень безопасности (например, SHA256). В рабочей среде рекомендуется получить доверенный сертификат из центра сертификации. В целях тестирования можно использовать самозаверяющие сертификаты. Чтобы создать самозаверяющий сертификат, см. раздел о [командлете Powershell New-SelfSignedCertificate](/powershell/module/pkiclient/new-selfsignedcertificate) или [команде certreq](/windows-server/administration/windows-commands/certreq_1). Инструкции по установке нового сертификата в удаленном экземпляре SQL Server см. в разделе [Включение зашифрованных соединений для ядра СУБД](/sql/database-engine/configure-windows/enable-encrypted-connections-to-the-database-engine.md).
