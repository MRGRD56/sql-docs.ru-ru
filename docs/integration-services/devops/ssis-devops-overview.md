...
## <a name="ssis-build-task-version-1-preview"></a>Задача сборки SSIS версии 1.* (предварительная версия)

Улучшения в версии 1.*:

- Устранена зависимость от Visual Studio и конструктора SSIS. Задача сборки может выполняться на агенте, размещаемом у Майкрософт, или на локальном агенте с ОС Windows и .NET Framework версии 4.6.2 или выше.

- Отсутствие необходимости устанавливать не входящие в комплект компоненты.

- Поддержка уровней защиты EncryptionWithPassword и EncryptionAllWithPassword.

### <a name="version-1-only-properties"></a>Эксклюзивные свойства версии 1.*

#### <a name="project-password"></a>Пароль проекта

Пароль для проекта SSIS и его пакетов. Этот аргумент допустим только в том случае, если для проекта и пакетов SSIS установлен уровень защиты EncryptSensitiveWithPassword или EncryptAllWithPassword. В модели развертывания пакета все пакеты должны иметь одинаковый пароль, который и указывается в этом аргументе.

#### <a name="strip-sensitive-data"></a>Исключение конфиденциальных данных

При значении true (истина) изменяет уровень защиты проекта SSIS на DontSaveSensitive. Если установлен уровень защиты EncryptSensitiveWithPassword или EncryptAllWithPassword, необходимо задать должным образом аргумент "Пароль проекта". Этот параметр допустим только для модели развертывания проекта.
...
## <a name="ssis-deploy-task-version-1-preview"></a>Задача развертывания SSIS версии 1.* (предварительная версия)
...
Пароль для расшифровки файлов ISPAC или DTSX. Этот аргумент действует только при уровне защиты EncryptSensitiveWithPassword или EncryptAllWithPassword.
...
|Свойство  |Описание  |Примечания  |
|---------|---------|---------|
|name|Имя параметра.|<li>Параметр может быть параметром проекта или параметром пакета. <li>Если параметр не существует, он пропускается. <li>Если параметр является свойством диспетчера подключений, имя должно иметь следующий формат: **CM.\<Connection Manager Name>.\<Property Name>** . |
|контейнер|Контейнер параметра.|<li>Если параметр является параметром проекта, *контейнер* должен быть именем проекта. <li>Если это параметр пакета, *контейнер* должен быть именем пакета с расширением **DTSX**.|
|value|Значение параметра.|<li>Если *valueType* имеет значение *referenced*: Значение является ссылкой на переменную среды в типе *string*. <li> Если *valueType* имеет значение *literal*: Этот атрибут поддерживает любые допустимые значения JSON типа *boolean*, *number* и *string*. <li> Значение будет преобразовано в тип целевого параметра. Если преобразование невозможно, произойдет ошибка.<li> Значение *NULL* является недопустимым. Задача пропустит этот объект параметра и выдаст предупреждение.|
...
### <a name="version-104"></a>Версия 1.0.4
...
### <a name="version-103"></a>Версия 1.0.3

Дата выпуска: 21 октября 2020 г.

- Разрешено указывать суффикс строки подключения для задачи развертывания служб SSIS и задачи настройки каталога служб SSIS.

### <a name="version-102"></a>Версия 1.0.2
